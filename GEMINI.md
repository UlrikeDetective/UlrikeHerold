...
- **Key Features:**
    ...
    - **Interactive Travel Map:** Located at `continent_map_pins.html`, powered by Leaflet.js and PapaParse.
...
- **Development Server:** For features like CSV loading in `continent_map_pins.html`, it is recommended to use a local live server (e.g., VS Code Live Server or `python -m http.server`).
- **Deployment:** The site is designed for hosting on GitHub Pages.

---

## Architecture & Data
- **Travel Map Data:** The markers on the travel map are dynamically loaded from `Data/mapPlaces.csv`.
...
- **Styling:** 
    - Prefer Vanilla CSS.
    - Typography uses `Cutive Mono`, `Pacifico`, and `Outfit` fonts (via Google Fonts).
    - Layouts utilize CSS Grid (`.parent`) and Flexbox (`.hero`, `.coding`).
- **Modularity:** Maintain the separation of sub-projects to ensure they remain standalone and functional.
- **Data Management:** Keep `Data/mapPlaces.csv` updated with travel locations, ensuring the column structure (`city`, `visits`, `latitude`, `longitude`, `reviews`, `continent`) is preserved.
